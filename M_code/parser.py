...
def extract_comparison_table(html: str):
    soup = BeautifulSoup(html, "html.parser")
...
def parse_pdp_page(html: str) -> Dict[str, Any]:
    ...
    if medicine_comparison is not None and not medicine_comparison.get("current_product"):
        cp = {}
        sp = {}
        if rsc:
    ...
    if manufacturer_info:
        result["manufacturer_info"] = manufacturer_info
    ...
    if category_hierarchy:
        result["category_hierarchy"] = category_hierarchy

    if medicine_comparison is not None:
        result["medicine_comparison"] = medicine_comparison
